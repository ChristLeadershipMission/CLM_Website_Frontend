...
const CampusManagementComponent = ({ hideSideBar }) => {
  ...
  const [campusDataList, setCampusDataList] = useState(
    FetchFromSessionStorage("campusDatas", [])
    );
          ...
          >
            {campusData.map((data) => {
              ...
              return (
                <div
                  key={id}
                  className={`lg:w-[100%] md:w-[90%] w-[85vw] relative pb-10 border border-black`}
                >
                  <div
                    className="bg-[#0A063E] lg:px-5 md:px-10 rounded-t-md border border-red
                "
                  >
                  ...
                  <div
                   ...
                   lg:py-3 py-3 gap-3 text-black rounded-b-md"
                    onClick={() => MoreInfo(id)}
                  >
                    <span
